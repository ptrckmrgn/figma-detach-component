...
const plugin = () => {
    ...
    if (selection.length > 0) {
        ...
        if (count > 0) {
            ...
            figma.notify(`${count} master component${count > 1 ? "s" : ""} successfully detached`);
        }
        else {
            figma.notify("No master components found in your selection");
        }
    }
    else {
        figma.notify("Select a master component to detach");
    }
    figma.closePlugin();
};
const getComponentNodes = (node) => {
    // If node has children, traverse
    if (node.type === "COMPONENT") {
        return [node];
    }
    else if ("children" in node && node.children.length > 0) {
        return node.children.reduce((all, child) => {
...
const createFrameFromComponent = (component) => {
